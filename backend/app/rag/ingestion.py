...
class IngestionService:
    ...
    def ingest_document(self, pdf_bytes: bytes, filename: str) -> dict[str, Any]:
        ...
        with db_manager.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id, chunk_count FROM documents WHERE file_hash = %s;", (file_hash,)
                )
        ...
        self._store_document_metadata(
            document_id=document_id, filename=filename, file_hash=file_hash, chunk_count=len(nodes)
        )

        #       6. Log Exact Embedding Telemetry
        logger.info(
            "Exact Embedding Telemetry Captured",
            extra={
        ...
        logger.info("Ingestion complete", extra={"document_id": document_id, "chunks": len(nodes)})
    ...
    def _store_document_metadata(
        self, document_id: str, filename: str, file_hash: str, chunk_count: int
    ):
